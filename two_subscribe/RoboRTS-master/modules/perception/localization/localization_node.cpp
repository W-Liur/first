...
ErrorInfo LocalizationNode::Init() {
  ...
  //laser_scan_sub_= nh_.subscribe(localization_config_.laser_topic_name(), 2, &LocalizationNode::LaserScanCallback, this);
  ...
  //wyy
  near0_enemy_pub_=nh_.advertise<geometry_msgs::Pose>("near0_enemy_",2,true);
  near1_enemy_pub_=nh_.advertise<geometry_msgs::Pose>("near1_enemy_",2,true);
...
void LocalizationNode::LaserScanCallback(const sensor_msgs::LaserScan::ConstPtr &laser_scan_ptr) {
  ...
  if (!laser_init_) {
    ...
    TransformLaserpose(laser_scan_msg, laser_pose_v);//base_laser_link to base_link
...
//	std::cout << "Latest TF" << std::endl;
  ...
  //wyy
  if(amcl_ptr_->laser_ptr_->near0_enemy_(0)>0 || amcl_ptr_->laser_ptr_->near0_enemy_(1)>0){
    geometry_msgs::Pose pose_msg;
    TransformEnemyPoseToMsg(amcl_ptr_->laser_ptr_->near0_enemy_, pose_msg);
    near0_enemy_pub_.publish(pose_msg);
  }
  if(amcl_ptr_->laser_ptr_->near1_enemy_(0)>0 || amcl_ptr_->laser_ptr_->near1_enemy_(1)>0){
    geometry_msgs::Pose pose_msg;
    TransformEnemyPoseToMsg(amcl_ptr_->laser_ptr_->near1_enemy_, pose_msg);
    near1_enemy_pub_.publish(pose_msg);
  }
}
...
//wyy
void LocalizationNode::TransformEnemyPoseToMsg(const math::Vec4d &enemy_pose, geometry_msgs::Pose &enemy_pose_msg) {
  enemy_pose_msg.position.x=enemy_pose(0);
  enemy_pose_msg.position.y=enemy_pose(1);
  enemy_pose_msg.position.z=enemy_pose(2);
  tf::quaternionTFToMsg(tf::createQuaternionFromYaw(enemy_pose(3)),enemy_pose_msg.orientation);
}
