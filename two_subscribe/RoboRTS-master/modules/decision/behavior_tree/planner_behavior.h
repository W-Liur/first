...
class PlannerBehavior: public ActionNode{
 ...
 private:
  virtual void OnInitialize() {
    LOG_ERROR<<"PlannerBehavior OnInitialize!";
    std::cout<<name_<<" "<<__FUNCTION__<<std::endl;
  };
/*<<<<<<<<<<<<<<<<<<李文浩修改<<<<<<<<<<<<<<<<<<<<<<<<<*/
  virtual BehaviorState Update() {
      LOG_ERROR<<"PlannerBehavior Update!";
      LOG_ERROR<<blackboard_ptr_->HasGoal();
    // First to check if there is a new goal to send
    if (blackboard_ptr_->HasGoal()){
      LOG_ERROR<<"PlannerBehavior Update!";
      if(blackboard_ptr_->HasGoalEnd()==1){
        geometry_msgs::PoseStamped poseZero;
        global_planner_goal_.goal1 = blackboard_ptr_->GetGoal1();
        global_planner_goal_.goal2 = poseZero;
      } else {
        global_planner_goal_.goal1 = blackboard_ptr_->GetGoal1();
        global_planner_goal_.goal2 = blackboard_ptr_->GetGoal2();
      }
      global_planner_goal_.goalend = blackboard_ptr_->HasGoalEnd();
...
/*>>>>>>>>>>>>>>>>>>李文浩修改>>>>>>>>>>>>>>>>>>>>>>>>>*/
  ...
  virtual void OnTerminate(BehaviorState state) {
    LOG_ERROR<<"PlannerBehavior OnTerminate!";
    switch (state){
