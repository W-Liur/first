...
//  GlobalPlannerConfig global_planner_config;
  ...
  goal_distance_tolerance_ = global_planner_config.goal_distance_tolerance_double();
...
//lwh
void GlobalPlannerNode::SetGoalEnd(int goalend) {
      goalend_=goalend;
    }

int GlobalPlannerNode::GetGoalEnd() {
  return goalend_;
}

geometry_msgs::PoseStamped GlobalPlannerNode::GetGoal1() {
  std::lock_guard<std::mutex> goal_lock(goal1_mtx_);
  return goal1_;
}

void GlobalPlannerNode::SetGoal1(geometry_msgs::PoseStamped goal) {
  std::lock_guard<std::mutex> goal_lock(goal1_mtx_);
  goal1_ = goal;
}
/*<<<<<<<<<<<<<<<<<<李文浩修改<<<<<<<<<<<<<<<<<<<<<<<<<*/
geometry_msgs::PoseStamped GlobalPlannerNode::GetGoal2() {
  std::lock_guard<std::mutex> goal_lock(goal2_mtx_);
  return goal2_;
}

void GlobalPlannerNode::SetGoal2(geometry_msgs::PoseStamped goal) {
  std::lock_guard<std::mutex> goal_lock(goal2_mtx_);
  goal2_ = goal;
}
/*>>>>>>>>>>>>>>>>>>李文浩修改>>>>>>>>>>>>>>>>>>>>>>>>>*/
...
//    }
    ...
    if(goalend==1){
      goal_distance_tolerance_ = global_planner_config.goal_distance_tolerance_single();
      ...
      if (error_info.IsOK()) {
        ...
        if (GetDistance(current_start, current_goal) < goal_distance_tolerance_
          ...
          LOG_ERROR<<"Have finished this plan!";
        }
      ...
      if(temp) {
        goal_distance_tolerance_ = global_planner_config.goal_distance_tolerance_double();
        error_info = global_planner_ptr_->Plan(current_start, current_goal, current_path);
        errorNext_info = global_planner_ptr_->Plan(current_goal,next_goal,next_path);
        current_path.insert(current_path.end(),next_path.begin(),next_path.end());
      } else {
        //goal_distance_tolerance_ = global_planner_config.goal_distance_tolerance_single();
        error_info = global_planner_ptr_->Plan(current_start, next_goal, current_path);
      }
      ...
      if (error_info.IsOK()) {
        ...
        if (GetDistance(current_start, next_goal) < goal_distance_tolerance_
            && GetAngle(current_start, next_goal) < goal_angle_tolerance_
                ) {
          temp = true;
          SetNodeState(NodeState::SUCCESS);
          LOG_ERROR<<"Have finished this plan!";
