...
//! if is the last point
  ...
  //! Goal mutex
  std::mutex goal1_mtx_;
/*<<<<<<<<<<<<<<<<<<李文浩修改<<<<<<<<<<<<<<<<<<<<<<<<<*/
  //! Received goal for global planner (Input)
  geometry_msgs::PoseStamped goal2_;
  //! Goal mutex
  std::mutex goal2_mtx_;
/*>>>>>>>>>>>>>>>>>>李文浩修改>>>>>>>>>>>>>>>>>>>>>>>>>*/
...
//lwh
   ...
   */
  void SetGoal1(geometry_msgs::PoseStamped goal);
/*<<<<<<<<<<<<<<<<<<李文浩修改<<<<<<<<<<<<<<<<<<<<<<<<<*/
/**
* @brief Check if the plan thread is still under execution, if not, start the plan thread.
* @return True if start the plan thread successful, else false.
 ...
 * @param goal planner goal
 */
  void SetGoal2(geometry_msgs::PoseStamped goal);
/*>>>>>>>>>>>>>>>>>>李文浩修改>>>>>>>>>>>>>>>>>>>>>>>>>*/
  /**
